...
class StackRedeployGitFormController {
  ...
  async submit() {
    ...
    const that = this;
    confirmStackUpdate(
      '对该堆栈或应用程序在 Portainer 中所做的任何本地更改将被覆盖，这可能导致服务中断。您确定要继续吗？',
      isSwarmStack
      ...
      try {
        ...
        that.Notifications.success('成功', '成功拉取并重新部署堆栈');
        that.$state.reload();
      } catch (err) {
        that.Notifications.error('失败', err, '堆栈重新部署失败');
      } finally {
  ...
  async saveGitSettings() {
    return this.$async(async () => {
      try {
        ...
        this.Notifications.success('成功', '成功保存堆栈设置');
